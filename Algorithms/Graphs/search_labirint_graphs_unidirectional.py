...
def search_labirint_graphs(quantity_nodes: int = 10,
                           first_node: list = (0, 1)):
    """
    Функция построения однонаправленного графа без узлов
    """
    import random, copy
    ...
    rand_trait = copy.copy(quantity_nodes) # необходимо если надо ставить входное число n-1

    limiter = 0  # переменная для ограничения по диагонали
    n = 0  # переменная для обхода по массиву по оси У
    def search_recurent(quantity_nodes, limiter, n):
        if quantity_nodes > 0:

            if rand_trait-1 > n and limiter <= rand_trait-1:
                for i in range(3):
                    k = random.randint(limiter, rand_trait-1)  # переменная для определения случайного индекса  в массиве по оси Х
                    if k == n or graph[-1][-1] == k:
                        continue
                    if array_visits[quantity_nodes-1][i] == True:
                        continue
                    array_visits[n][k] = True
                    node = (n, k)
                    graph.append(node)
                    array_adj[n][k] = node

                search_recurent(quantity_nodes, limiter+1, n+1)

    search_recurent(quantity_nodes, limiter, n)
